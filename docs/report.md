...
Study Research Agent is a Python AI-assisted command-line system. Its goal is to help a user analyze study notes, project files, CSV records, or JSON data by using tools during execution. The user gives a question, a file path, and optionally a calculation. The agent reads and converts the file, searches for relevant evidence, calculates when requested, and returns a meaningful answer. When `--ai` is enabled, the agent also uses the OpenAI API to synthesize the final answer from the tool results.
...
- If AI mode is enabled, it calls the OpenAI synthesizer tool.
...
- Optional AI mode through the `--ai` flag.
...
6. The OpenAI synthesizer rewrites the final answer if `--ai` is enabled.
...
AI mode should be released as an optional feature. Users should explicitly set `OPENAI_API_KEY`, and the project should continue to support offline fallback for testing and demonstrations.
...
How tools are integrated: the agent receives the user request, calls `FileReaderTool` if a file is present, passes the resulting chunks to `TextSearchTool`, calls `CalculatorTool` if a calculation is present, and joins the outputs into one response. If `--ai` is enabled, `OpenAISynthesizerTool` creates the final API-backed answer from the same evidence.
